...
app.post('/home/pick_color', function(req, res) {
	...
	var color_select = 'SELECT (hex_value) FROM favorite_colors;';
...
/* /team_stats - get request (no parameters)
  ...
  app.get('/team_stats', function(req, res){
  	var get_football_games = 'SELECT * FROM football_games;';
    ...
    var losing_games = 'SELECT count(*) FROM football_games WHERE home_score < visitor_score;'
  	...
  	.then(query_results => {
  		res.render(
  			...
  			{
  				my_title: 'Team Stats',
  				gameInfo: query_results[0],
  				gamesWon: query_results[1],
  				gamesLost: query_results[2]
  			}
  		)
  	})
  	.catch(error => {
...
/*
  ...
  app.get('/player_info', function(req, res){
    var player_names = 'SELECT (name, id) FROM football_players;';
    db.task('load-team-stats', task =>
      task.batch([
        task.any(player_names)
      ])
    )
    .then(query_results => {
      res.render(
